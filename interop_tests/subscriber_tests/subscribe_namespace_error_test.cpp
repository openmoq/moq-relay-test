...
public:
    explicit SubscribeNamespaceErrorTest(const TestContext &context) : BaseTest(context) {}
    ~SubscribeNamespaceErrorTest() override = default;
    std::string getName() const override { return "SubscribeNamespaceErrorTest"; }
    std::string getDescription() const override {
        return "Verifies that subscribing to a non-announced namespace fails appropriately";
    }
...
TestResult SubscribeNamespaceErrorTest::execute() {
    log("Attempting to subscribe to namespace without announcing: " + trackNamespace_);
    ...
    assertFalse(subscribeResult, "Subscribe request should fail for non-announced namespace");
